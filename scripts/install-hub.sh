...
ENV_PATH="/etc/hub/hub.env"
UNIT_PATH="/etc/systemd/system/hub.service"
DATA_DIR="/var/lib/hub"
BIND="0.0.0.0:3737"
START_SERVICE=1

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "${SCRIPT_DIR}/.." && pwd)"
source "${REPO_ROOT}/branding.conf"
BINARY="${REPO_ROOT}/target/release/hub"

usage() {
    cat <<'EOF'
Installiert Hub als systemd-Dienst (Debian/Proxmox).

Aufruf:
  sudo ./scripts/install-hub.sh [Optionen]

Optionen:
  --binary PATH    Hub-Binary (Standard: target/release/hub)
  --bind ADDR      Bind-Adresse (Standard: 0.0.0.0:3737)
  --data-dir PATH  Persistente Daten (Standard: /var/lib/hub)
...
cat >"${ENV_TMP}" <<EOF
HUB_BIND=${BIND}
HUB_ALLOW_NETWORK=${ALLOW_NETWORK}
