...
# (docs/branding/product-branding-sheet.png, 1536x1024).
#
...
SHEET="$ROOT_DIR/docs/branding/product-branding-sheet.png"
OUT="$ROOT_DIR/studio/native/assets/icon"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
magick "$TMP/haupt.png" -resize 512x512 "$OUT/studio-512.png"
magick "$TMP/haupt.png" -resize 256x256 "$OUT/studio-256.png"
...
for s in 48 64 128; do
  magick "$TMP/fluegel-512.png" -resize "${s}x${s}" "$OUT/studio-$s.png"
done
...
for s in 16 24 32; do
  ...
  magick "$TMP/bg$s.png" \
    ...
    -gravity center -compose over -composite "$OUT/studio-$s.png"
done
