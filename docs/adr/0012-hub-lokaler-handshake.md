# ADR 0012: Hub als optionaler lokaler Koordinationsdienst
...
Hub soll Projektstände zwischen Office und Workshop verteilen, die
...
**Studio first, Hub optional.** Editor, lokales Speichern und Laserbetrieb
müssen ohne Hub möglich bleiben.

Der sichere Standard läuft weiterhin auf demselben Rechner wie Studio. Für
...
Hub beginnt als **optional aktivierter lokaler HTTP-Dienst**. Der aktuelle
...
- Standardbindung: `127.0.0.1:3737`; eine Nicht-Loopback-Bindung verlangt
  sowohl `HUB_BIND` als auch die ausdrückliche Freigabe
  `HUB_ALLOW_NETWORK=1`;
...
- die native Anwendung erhält eine globale Hub-Einstellung mit Aktivierung,
...
- ein nicht gestarteter oder nicht erreichbarer Hub beeinträchtigt weder
...
1. Hub steuert niemals direkt eine Maschine und besitzt keinen
   `MachineDriver`.
2. Hub ist kein Speicher-Wahrheitszentrum für den Editor. Lokale Dateien und
...
4. Eine Bindung außerhalb des Loopback-Interfaces ist eine bewusste
   Betriebsentscheidung und benötigt `HUB_ALLOW_NETWORK=1`. Solange
...
5. Handshake-Kompatibilität wird über die Protokollversion entschieden, nicht
   über die Hub-Binaryversion.
6. **Lokales Speichern kommt zuerst.** Ein Speichervorgang schreibt immer zuerst
   die lokale Projektversion und endet unabhängig vom Hub-Ergebnis
...
7. **Hub verteilt Versionen, verändert sie aber nicht.** Hub speichert
...
8. **Der empfangende Client entscheidet.** Neue Versionen landen zunächst in
   ...
   Projekten zeigt Studio `Übernehmen`, `Später`, `Änderungen anzeigen` und
   später einen expliziten Merge-Ablauf. Konflikte bleiben als parallele
   Versionszweige erhalten; Hub bestimmt keinen Gewinner.
...
10. **Ruida-Exklusivität ist Koordination, keine Maschinensteuerung.** Hub
   vergibt genau eine zeitlich begrenzte Lease pro Ethernet-Controller.
   Nur der Lease-Inhaber verbindet sich selbst mit dem Ruida; Hub sendet
...
11. **Die Verbindung bleibt manuell.** `Verbinden` im Laser-Tab fordert die
   ...
   Hub ihn per Push zum Trennen auf und übergibt anschließend die Lease.
...
13. **Hub-Ausfall bleibt beherrschbar.** Ist Hub nicht konfiguriert, darf
   Studio direkt verbinden. Ist er konfiguriert, aber nicht erreichbar, warnt
   Studio vor einer unkoordinierten Ethernet-Verbindung und verlangt eine
    manuelle Bestätigung. USB-Verbindungen benötigen keine Hub-Lease.
14. **Die Asset-Bibliothek ist lokal vollständig.** Name, automatisch
    ...
    Hub. Hub synchronisiert nur Asset-Bytes und zusammenführbare
...
- Maschinen-Queueing oder Jobübertragung durch Hub;
...
- Hub bindet standardmäßig an `127.0.0.1:3737`. Eine interne
  Netzwerkbindung ist nur mit `HUB_BIND=<adresse>:3737` und zusätzlichem
  `HUB_ALLOW_NETWORK=1` möglich; ohne Opt-in verweigert der Start die
  Freigabe. `hub/README.md` dokumentiert den eingeschränkten
  Proxmox-/systemd-Testbetrieb; `scripts/install-hub.sh` installiert und
...
- der Client liegt in `studio-application`, validiert URL, HTTP-Status,
...
- Aktivierung, URL und Verbindungstest liegen in der globalen
  Hub-Einstellungssektion; alte Settings erhalten sichere Defaults;
...
- der Verbindungstest registriert den Arbeitsplatz und zeigt Hubs bekannten
  Anwesenheits-Snapshot;
- bei aktivierter Hub-Koordination meldet sich Studio regelmäßig aus einem
...
- Hub bietet zusätzlich `GET /api/v1/events/projects` als cursorbasierten
  ...
  nach vier Sekunden endet ein unverändertes Warten regulär. Studio führt den
...
- der Long-Poll ersetzt nicht Heartbeat, Outbox oder Inbox: Er verkürzt nur die
  ...
  Hub-Neustart erkennt Studio die neue Server-Instanz-ID und startet den
  Cursor gefahrlos erneut bei null;
- Hub hält die Registrierung vorerst nur im Arbeitsspeicher. Ein Neustart
  leert die Anwesenheitsliste, die laufenden Clients melden sich selbstständig
  wieder an;
- `scripts/run-local-hub-demo.sh` startet Hub, Office und Workshop mit
  voneinander isolierten Datenverzeichnissen in drei Terminals;
- nach jedem erfolgreichen lokalen Speichern legt Studio bei aktiviertem
  Hub einen atomar geschriebenen Outbox-Eintrag unter
  `sync/outbox/<revision_id>/` an. Manifest und eigene `payload.laserproj`-Kopie
...
- Hub prüft den Inhaltshash und speichert Manifest und Payload atomar unter
  `projects/<project_id>/revisions/<revision_id>/`. Die Ablagewurzel ist über
  `HUB_DATA_DIR` konfigurierbar;
...
- Hub liefert einem Arbeitsplatz ausschließlich Revisionen anderer
  Arbeitsplätze. Studio prüft deren Hash und legt sie idempotent und atomar
...
- erst nach erfolgreicher, hashgeprüfter Inbox-Ablage bestätigt Studio die
  Revision bei Hub. Hub speichert den Beleg pro Arbeitsplatz atomar unter
...
- der Projekt-Reiter besitzt den Bereich `Von Hub`; ein Badge am
...
- Thumbnail-PNGs sind jederzeit neu erzeugbare lokale Cache-Dateien. Sie werden
  nicht über Hub übertragen. Tag-Metadaten werden beim Asset-Sync als
...
- Hub bietet den Katalog über `GET /api/v1/assets`,
...
- Projekt- und Asset-Katalog werden beim Start einmal geladen und im nativen
  ...
  Importen oder Hub-Downloads aktualisiert; der UI-Framepfad liest und parst
...
- `Änderungen anzeigen` öffnet einen strikt read-only Vergleich. Lokales Projekt
  und Hub-Revision werden über die stabile Projekt-ID zugeordnet und mit
...
- `Lokale Version behalten` quittiert ausschließlich die konkrete Inbox-
  Revision als ignoriert. `Hub-Version übernehmen` hängt den empfangenen
...
- Hub hält je Arbeitsplatz Sicherungen von `ui_settings`, `laser_profiles`
...
- Studio übergibt lokale Änderungen an Settings und Profilkatalogen nur dem
  vorhandenen Hintergrundthread. Netzwerkzugriff findet weder beim Speichern
  noch im egui-Callback statt und ein Hub-Fehler macht die lokale Änderung
  nicht rückgängig;
- der Hub-Dialog lädt vorhandene Sicherungen ausdrücklich auf
...
- Ist Hub für ein Ethernet-Profil aktiviert, aber nicht erreichbar, verlangt
...
- Ethernet-Profile leiten aus der Zieladresse eine arbeitsplatzunabhängige
  ...
  Bestätigung verbindet Studio selbst den Maschinentreiber;
- Hub hält Leases ausschließlich als Koordinationszustand im Speicher. Ihre
  Laufzeit beträgt 15 Sekunden, Studio erneuert sie alle fünf Sekunden über
  `POST /api/v1/leases/heartbeat`; Hub besitzt weiterhin keinen Treiber und
...
- eine abgelaufene, zuletzt untätige Lease darf Hub automatisch neu
  vergeben. Bei einem abgelaufenen nicht sicheren Zustand verlangt Studio vor
...
- Trennen, Profilwechsel und Konfigurationswechsel geben eine aktive Lease
  bestmöglich frei. Geht Heartbeat oder Lease verloren, trennt Studio den
...
- Hintergrundberichte werden im UI strikt in Empfangsreihenfolge verarbeitet.
  ...
  bleiben;
- Der Bereich `Von Hub` kann alle offenen und zurückgestellten Revisionen
...
- die Asset-Bibliothek zeigt ihre virtualisierten Treffer als Kartenraster.
  ...
  verhindert den unmittelbaren erneuten Download durch Hub, wird aber bei
...
optional ein späterer Objekt-Merge. Hub darf Versionen verteilen und
...
Änderung einen geordneten Snapshot. Hub dedupliziert identische Inhalte
...
Hub meldet die vorhandenen Projekt-, Versions- und Inhaltshash-Tupel.
Studio stellt für jede lokal gespeicherte Projektversion einen dauerhaften
Outbox-Snapshot sicher und überträgt alle auf Hub fehlenden Revisionen erneut.
Damit lässt sich auch ein leerer oder nur teilweise vorhandener Hub-Bestand
