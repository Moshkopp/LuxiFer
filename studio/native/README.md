# studio — nativer Editor (winit + wgpu + egui)

Der Umbau aus **ADR 0010**: die Zeichen-/UI-Schicht von Studio nativ, ohne
WebView, ohne IPC. `studio-core` bleibt die einzige Quelle der Wahrheit und wird
direkt gelinkt. Läuft **neben** der weiter funktionierenden Tauri-App
(`studio/frontend/`), damit man vergleichen kann.

## Warum

Direkter Release-Vergleich mit ThorBurn (Qt) zeigte Studio auf allen Achsen
...
```bash
# aus der Repo-Wurzel
GDK_BACKEND=x11 cargo run -p studio --release

# mit direkt geladener Datei (erstes Argument):
GDK_BACKEND=x11 cargo run -p studio --release -- /pfad/zu/datei.svg
```
...
Das Ergebnis liegt unter `dist/Studio-<version>-<architektur>.AppImage`.

`GDK_BACKEND=x11` aus demselben Grund wie in `dev.sh` (Wayland-Present-Latenz).

### Test-Umgebungsvariablen
- `STUDIO_FILL=1` — beim Auto-Import gleich alle Layer auf Fill stellen.
- `STUDIO_TAB=laser` — rechten Reiter direkt auf Laser starten.
...
`studio-core` (199 Core-Tests unberührt; native Verdrahtung mit 8 eigenen Tests).
