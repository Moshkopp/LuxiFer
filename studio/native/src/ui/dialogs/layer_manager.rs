...
pub(in crate::ui) fn layer_manager_window(
    ...
    laser: Option<&studio_core::LaserProfile>,
...
fn layer_table_row(
    ui: &mut egui::Ui,
    index: usize,
    layer: &mut studio_application::LayerParams,
    color: [u8; 3],
) {
    ui.horizontal(|ui| {
        ui.spacing_mut().item_spacing.x = COL_GAP;
...
fn mode_combo(ui: &mut egui::Ui, index: usize, mode: &mut studio_core::LayerMode) {
    if *mode == studio_core::LayerMode::Image {
        ui.label("Bildgravur");
        return;
    }
    let label = |mode| match mode {
        studio_core::LayerMode::Cut => "Schneiden",
        studio_core::LayerMode::Fill => "Gravieren",
        studio_core::LayerMode::Raster => "Raster",
        studio_core::LayerMode::Image => "Bildgravur",
    };
...
fn process_detail(ui: &mut egui::Ui, layer: &mut studio_application::LayerParams) {
    match layer.mode {
        studio_core::LayerMode::Cut => {
            ui.weak("—");
        }
        ...
        studio_core::LayerMode::Raster | studio_core::LayerMode::Image => {
            ui.horizontal(|ui| {
                ui.add(
