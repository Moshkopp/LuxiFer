    ...
    ui.allocate_ui(egui::vec2(ui.available_width(), 26.0), |ui| {
        ui.columns(3, |columns| {
            ...
            columns[2].with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if hub_enabled {
                    ui.separator();
                    let (color, hint) = match hub_status {
                        HubTestStatus::Syncing(_) => (
                            egui::Color32::from_rgb(0xfb, 0xbf, 0x24),
                            "Hub synchronisiert gerade",
                        ),
                        HubTestStatus::Connected(_) => (
                            egui::Color32::from_rgb(0x4a, 0xde, 0x80),
                            "Hub ist verbunden",
                        ),
                        HubTestStatus::Failed(_) => {
                            (ui.visuals().error_fg_color, "Hub ist nicht erreichbar")
                        }
                        HubTestStatus::Idle => {
                            (ui.visuals().error_fg_color, "Hub ist nicht verbunden")
                        }
                    };
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::Label::new(RichText::new("Hub").color(color))
                                    .sense(egui::Sense::click()),
                            )
                            .on_hover_cursor(egui::CursorIcon::PointingHand)
                            .on_hover_text(format!("{hint} · Hub-Eingang öffnen"))
                            .clicked()
                        {
                            actions.push(UiAction::OpenHubInbox);
                        }
                    });
                }
                ui.separator();
