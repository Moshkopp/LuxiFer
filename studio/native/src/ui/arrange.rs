...
pub(super) fn arrange_bar(
    ui: &mut egui::Ui,
    selection: usize,
    bbox: Option<studio_core::BBox>,
    size: &mut SelectionSizeState,
) -> Vec<UiAction> {
    use studio_core::{Align, Distribute};
...
fn sync_draft(state: &mut SelectionSizeState, bbox: Option<studio_core::BBox>) {
    state.source = bbox;
...
mod tests {
    ...
    fn proportionaler_breitenwert_berechnet_hoehe() {
        let bbox = studio_core::BBox::new(0.0, 0.0, 100.0, 50.0);
