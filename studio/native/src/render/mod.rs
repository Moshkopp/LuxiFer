    ...
    fn collect(&mut self, device: &wgpu::Device) -> Vec<f64> {
        ...
        for slot in &mut self.slots {
            ...
            match receiver.try_recv() {
                ...
                Ok(Err(error)) => {
                    log::warn!(target: "studio_render_perf", "GPU timestamp readback failed: {error}");
                    slot.receiver = None;
                }
...
impl Renderer {
    ...
    pub fn new(gpu: Gpu, egui_state: egui_winit::State) -> Self {
        let perf_enabled = std::env::var_os("STUDIO_RENDER_PROFILE").is_some();
