...
struct ProjectIntegrationResult {
    ...
    error: Option<studio_application::AppError>,
}

impl ProjectIntegrationRuntime {
    pub fn new() -> Result<Self, studio_application::AppError> {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                while let Ok(revision_ids) = request_rx.recv() {
                    ...
                    for revision_id in revision_ids {
                        let comparison =
                            match studio_application::compare_inbox_revision(&revision_id) {
                        ...
                        let accepted = if comparison.local_project_name.is_some() {
                            studio_application::accept_inbox_revision(&revision_id)
                        } else {
                            studio_application::apply_inbox_revision(&revision_id)
                        };
                        match accepted {
                            Ok(name) => {
                                result.accepted += 1;
            ...
            .map_err(|error| {
                studio_application::AppError::wrap(
...
impl App {
    ...
    pub fn defer_inbox_revision(&mut self, revision_id: &str) {
        match studio_application::defer_inbox_revision(revision_id) {
            Ok(()) => {
    ...
    pub fn reconsider_inbox_revision(&mut self, revision_id: &str) {
        match studio_application::reconsider_inbox_revision(revision_id) {
            Ok(()) => {
                self.refresh_project_inbox();
                self.toasts
    ...
    pub fn apply_all_inbox_revisions(&mut self) {
        let revision_ids: Vec<_> = self
            ...
            .filter(|entry| {
                matches!(
                    entry.status,
                    studio_application::InboxStatus::PendingReview
                        | studio_application::InboxStatus::Deferred
                )
            })
        ...
        let touches_open_project = revision_ids.iter().any(|revision_id| {
            studio_application::compare_inbox_revision(revision_id)
                .ok()
    ...
    fn start_project_integration(&mut self, revision_ids: Vec<String>) {
            ...
            .is_ok()
        {
            ...
            self.toasts
                .success("Hub-Projekte werden im Hintergrund übernommen …");
        }
    }
    ...
    pub fn poll_project_integration(&mut self) -> bool {
        ...
        if result.accepted > 0 {
            self.toasts
                .success(format!("{} Hub-Revision(en) übernommen.", result.accepted));
        }
    ...
    pub fn keep_local_inbox_revision(&mut self, revision_id: &str) {
        match studio_application::ignore_inbox_revision(revision_id) {
            Ok(()) => {
                self.refresh_project_inbox();
                self.toasts
    ...
    pub fn accept_inbox_revision(&mut self, revision_id: &str) {
        let target_is_open = match studio_application::compare_inbox_revision(revision_id) {
            Ok(comparison) => comparison
                .local_project_name
    ...
    pub fn refresh_project_inbox(&mut self) {
        match studio_application::list_inbox() {
    ...
    pub fn project_export(&mut self, name: &str) {
        let Some(target) = rfd::FileDialog::new()
            .add_filter(studio_core::branding::PROJECT_FILE_LABEL, &["laserproj"])
            .set_file_name(format!("{name}.laserproj"))
            .save_file()
        else {
            return;
        };
    ...
    fn queue_project_for_hub(&mut self) {
            ...
            .queue_current_for_sync(&self.ui_settings.workplace_id)
        {
            self.toasts.error(format!(
                "Projekt lokal gespeichert, Hub-Vormerkung fehlgeschlagen: {}",
                error.message()
            ));
        }
    }
