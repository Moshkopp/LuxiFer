...
use studio_core::geometry::{rotate_point, Pt};
...
pub(crate) fn world_outline(shape: &studio_core::model::Shape) -> (Vec<(f64, f64)>, bool) {
...
fn solid_fills_partitioned(
    ...
    for (li, layer) in state.layers.iter().enumerate() {
        ...
        if !layer.visible || !layer.mode.is_filled() || layer.mode == studio_core::LayerMode::Image
        {
            continue;
        }
...
pub fn bed_base(w: f32, h: f32, origin: studio_core::BedOrigin) -> Vec<Vertex> {
...
fn push_origin_marker(v: &mut Vec<Vertex>, origin: studio_core::BedOrigin, bed: (f64, f64)) {
    let first = v.len();
    let arm = 24.0;
    let cross = 8.0;
    let diamond = 4.0_f64;
...
mod tests {
    ...
    fn layer_manager_moduswechsel_erzeugt_im_canvas_einen_fill_batch() {
        ...
        let mut session = studio_application::EditorSession::new(state);
        let mut params = vec![studio_application::LayerParams::from_layer(
            &session.layers[0],
        )];
        params[0].mode = studio_core::LayerMode::Fill;
    ...
    fn offene_kontur_erzeugt_keine_design_flaeche() {
        let mut state = AppState::new();
        state.add_shape(studio_core::Geo::Polyline {
