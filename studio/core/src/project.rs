...
//! §3): ein Ordner pro Projekt, darin `projekt.laserproj` (JSON) mit Layer- und
...
pub const PROJECT_FILE: &str = "projekt.laserproj";
...
impl ProjectFile {
    ...
    /// für Anzeige, Aufräumen verwaister Assets und Hubs spätere
    ...
    /// `current_version` (altes Format: Geometrie in `projekt.laserproj`, Historie ohne
    ...
    fn migrate(&mut self) {
        ...
        // Kein gültiger Zeiger: Der Stand in projekt.laserproj wird zur aktuellen
    ...
    fn write_version_snapshot(
        ...
        let snap = vdir.join(format!("{version_id}.laserproj"));
    ...
    ///   wieder in-place).
    ///
    /// Schreibt `projekt.laserproj` und Snapshot + Thumbnail der betroffenen Version
    ...
    /// Löscht eine Version (`versions/<id>.laserproj` + `.png`) und entfernt sie aus
    ...
    pub fn delete_version(
        ...
        let vdir = projects_dir.join(&self.name).join(VERSIONS_DIR);
        let _ = std::fs::remove_file(vdir.join(format!("{version_id}.laserproj")));
        ...
        if was_current {
            ...
            // Geometrie der neuen aktuellen Version in projekt.laserproj übernehmen.
...
/// Basis-Datenverzeichnis. Reihenfolge: `STUDIO_DATA_DIR` → technischer
/// XDG-Anwendungsordner → Home-Fallback → `.` (Notnagel). Plattformneutral.
pub fn data_root() -> PathBuf {
    ...
    if let Ok(xdg) = std::env::var("XDG_DATA_HOME") {
        if !xdg.is_empty() {
            return PathBuf::from(xdg).join(crate::branding::DATA_DIR_NAME);
        }
    }
    if let Ok(home) = std::env::var("HOME") {
        if !home.is_empty() {
            return PathBuf::from(home)
                .join(".local")
                .join("share")
                .join(crate::branding::DATA_DIR_NAME);
        }
    }
    PathBuf::from(".")
}
...
mod tests {
    ...
    fn save_und_load_ueber_tempdir() {
        let dir = std::env::temp_dir().join(format!("studio_test_{}", std::process::id()));
    ...
    fn migrate_altes_format_erzeugt_v1() {
        ...
        let dir = std::env::temp_dir().join(format!("studio_mig_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn strg_s_aktualisiert_aktuelle_version_in_place() {
        ...
        let dir = std::env::temp_dir().join(format!("studio_scur_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
    ...
    fn shift_strg_s_legt_naechste_version_an_und_wird_aktuell() {
        let dir = std::env::temp_dir().join(format!("studio_ver_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
        let mut pf = ProjectFile::from_state(&s, "Proj", vec![]);
        pf.save_to_dir(&dir).unwrap();
    ...
    fn strg_s_auf_alter_version_verzweigt_dann_in_place() {
        let dir = std::env::temp_dir().join(format!("studio_br_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
    ...
    fn labels_zaehlen_auch_nach_loeschen_korrekt_hoch() {
        let dir = std::env::temp_dir().join(format!("studio_lbl_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
        let mut pf = ProjectFile::from_state(&s, "Proj", vec![]); // V1
        pf.save_to_dir(&dir).unwrap();
    ...
    fn version_loeschen_letzte_ist_geschuetzt() {
        let dir = std::env::temp_dir().join(format!("studio_del1_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
        let mut pf = ProjectFile::from_state(&s, "Proj", vec![]);
        pf.save_to_dir(&dir).unwrap();
    ...
    fn aktuelle_version_loeschen_befoerdert_vorherige() {
        let dir = std::env::temp_dir().join(format!("studio_del2_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let mut s = AppState::new();
        s.add_shape(Geo::Rect {
            x: 0.0,
    ...
    fn rename_erhaelt_id_und_verschiebt_ordner() {
        let dir = std::env::temp_dir().join(format!("studio_ren_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
    ...
    fn delete_entfernt_projekt() {
        let dir = std::env::temp_dir().join(format!("studio_del_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let s = state_with_two_layers();
