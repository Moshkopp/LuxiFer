...
pub(crate) fn enqueue_catalog_profile<T: Serialize>(
    ...
    let content_hash =
        studio_core::assets::content_hash(payload.as_deref().unwrap_or_default().as_bytes());
...
pub fn seed_shared_catalog(
    lasers: &[studio_core::LaserProfile],
    materials: &[studio_core::MaterialProfile],
) -> Result<(), AppError> {
...
fn state_path() -> PathBuf {
    studio_core::data_root().join(STATE_FILE)
}
