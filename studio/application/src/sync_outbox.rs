//! Persistente lokale Projekt-Outbox für Hub (ADR 0012).
//!
...
use studio_core::{assets::content_hash, data_root, datetime, ProjectFile};
...
pub fn enqueue_project_snapshot(
    ...
    let payload = std::fs::read(snapshot_path).map_err(|error| {
        AppError::wrap(
            ...
            "Gespeicherter Projektstand konnte nicht für Hub vorgemerkt werden.",
            error.to_string(),
        )
    })?;
    let root = outbox_dir();
    ...
    let result = (|| {
        ...
        let manifest = serde_json::to_vec_pretty(&entry).map_err(|error| {
            AppError::wrap(
                "outbox_json",
                "Hub-Outbox konnte nicht serialisiert werden.",
                error.to_string(),
            )
        })?;
    ...
    for info in studio_core::list_projects(&projects_dir) {
        ...
        for version in &project.versions {
            let snapshot = projects_dir
                ...
                .join(format!("{}.laserproj", version.id));
...
pub(crate) fn set_outbox_status(
    ...
    let mut entry: OutboxEntry = serde_json::from_slice(&bytes).map_err(|error| {
        AppError::wrap(
            "outbox_json",
            "Hub-Outbox enthält ungültige Daten.",
            error.to_string(),
        )
    })?;
    entry.status = status;
...
fn outbox_write_error(error: std::io::Error) -> AppError {
    AppError::wrap(
        "outbox_write",
        "Hub-Outbox konnte nicht geschrieben werden.",
        error.to_string(),
    )
}
...
fn outbox_read_error(error: std::io::Error) -> AppError {
    AppError::wrap(
        "outbox_read",
        "Hub-Outbox konnte nicht gelesen werden.",
        error.to_string(),
    )
}
