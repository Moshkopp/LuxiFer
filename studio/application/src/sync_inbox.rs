//! Persistente lokale Inbox für unveränderte Hub-Projektrevisionen.

use std::path::{Path, PathBuf};

use serde::{Deserialize, Serialize};
use studio_core::{assets::content_hash, data_root, datetime};

use crate::{hub::HubRevision, AppError};

const INBOX_DIR: &str = "sync/inbox";
const MANIFEST_FILE: &str = "manifest.json";
const PAYLOAD_FILE: &str = "payload.laserproj";
...
pub struct InboxComparison {
    ...
    pub local_state: Option<studio_core::state::AppState>,
    pub remote_state: studio_core::state::AppState,
...
pub(crate) fn store_remote_revision(revision: HubRevision) -> Result<bool, AppError> {
    if !valid_id(&revision.revision_id) || !valid_id(&revision.project_id) {
        return Err(AppError::new(
            "inbox_revision_id",
            "Hub lieferte eine ungültige Revisionskennung.",
        ));
    }
    ...
    if actual_hash != revision.content_hash {
        return Err(AppError::new(
            "inbox_hash",
            "Die von Hub gelieferte Projektrevision ist beschädigt.",
        ));
    }
    let root = inbox_dir();
    ...
    if manifest_path.exists() {
        ...
        return Err(AppError::new(
            "inbox_conflict",
            "Hub lieferte dieselbe Revisions-ID mit anderem Inhalt.",
        ));
    }
    ...
    let result = (|| {
        ...
        let manifest = serde_json::to_vec_pretty(&entry).map_err(|error| {
            AppError::wrap(
                "inbox_json",
                "Hub-Inbox konnte nicht serialisiert werden.",
                error.to_string(),
            )
        })?;
...
pub fn accept_inbox_revision(revision_id: &str) -> Result<String, AppError> {
    ...
    incoming_version.id = studio_core::datetime::gen_id();
    incoming_version.label = next_version_label(&local.versions);
    incoming_version.note = format!("Von Hub: {}", entry.source_workplace_id);
    ...
    let versions_dir = project_dir.join(studio_core::project::VERSIONS_DIR);
    ...
    let version_path = versions_dir.join(format!("{}.laserproj", local.current_version));
    let result = (|| {
        ...
        std::fs::rename(
            &project_temp,
            project_dir.join(studio_core::project::PROJECT_FILE),
        )
...
pub fn apply_inbox_revision(revision_id: &str) -> Result<String, AppError> {
    ...
    std::fs::create_dir_all(temp_dir.join(studio_core::project::VERSIONS_DIR))
        .map_err(inbox_write_error)?;
    let result = (|| {
        ...
        std::fs::write(temp_dir.join(studio_core::project::PROJECT_FILE), &bytes)
            .map_err(inbox_write_error)?;
        std::fs::write(
            temp_dir
                .join(studio_core::project::VERSIONS_DIR)
                .join(format!("{}.laserproj", project.current_version)),
            bytes,
        )
        .map_err(inbox_write_error)?;
        std::fs::rename(&temp_dir, &final_dir).map_err(inbox_write_error)
...
fn read_verified_project(entry: &InboxEntry) -> Result<studio_core::ProjectFile, AppError> {
    ...
    let project = studio_core::ProjectFile::from_json(&payload).map_err(|error| {
        AppError::wrap(
            "inbox_project_json",
...
fn unique_project_name(projects_dir: &Path, preferred: &str) -> String {
    ...
    for suffix in 1.. {
        let candidate = if suffix == 1 {
            format!("{preferred} (Hub)")
        } else {
            format!("{preferred} (Hub {suffix})")
        };
...
fn next_version_label(versions: &[studio_core::project::VersionInfo]) -> String {
    let max = versions
        .iter()
...
fn read_entry(path: &Path) -> Result<InboxEntry, AppError> {
    ...
    serde_json::from_slice(&bytes).map_err(|error| {
        AppError::wrap(
            "inbox_json",
            "Hub-Inbox enthält ungültige Daten.",
            error.to_string(),
        )
    })
}
...
fn inbox_write_error(error: std::io::Error) -> AppError {
    AppError::wrap(
        "inbox_write",
        "Hub-Inbox konnte nicht geschrieben werden.",
        error.to_string(),
    )
}
...
fn inbox_read_error(error: std::io::Error) -> AppError {
    AppError::wrap(
        "inbox_read",
        "Hub-Inbox konnte nicht gelesen werden.",
        error.to_string(),
    )
}
...
mod tests {
    ...
    fn revision(payload: &str) -> HubRevision {
        HubRevision {
    ...
    fn neues_projekt_wird_uebernommen_ohne_editorzustand_zu_beruehren() {
        ...
        let project = studio_core::ProjectFile::from_state(
            &studio_core::AppState::new(),
            "Vom Office",
            Vec::new(),
        );
        let payload = project.to_json().unwrap();
        let item = HubRevision {
    ...
    fn projekt_mit_vorhandenem_asset_wird_uebernommen() {
        ...
        let asset_id = studio_core::assets::content_hash(bytes);
        let meta = studio_core::AssetMeta {
            id: asset_id.clone(),
            ext: "bin".into(),
            kind: studio_core::AssetKind::Image,
        ...
        let project = studio_core::ProjectFile::from_state(&state, "Mit Asset", Vec::new());
        let payload = project.to_json().unwrap();
    ...
    fn vergleich_findet_lokales_projekt_ueber_stabile_id_und_mutiert_nichts() {
        ...
        let local = studio_core::ProjectFile::from_state(
            &studio_core::AppState::new(),
            "Werkstattname",
            Vec::new(),
        );
