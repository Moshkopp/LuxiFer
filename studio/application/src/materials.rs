...
use studio_core::{MaterialLibrary, MaterialProcess, MaterialProcessDefaults, MaterialProfile};
...
impl MaterialService {
    pub fn load() -> Result<Self, AppError> {
        Self::load_from(&studio_core::data_root())
    }
...
mod tests {
    ...
    fn layer(mode: studio_core::LayerMode) -> LayerParams {
        let mut layer = LayerParams::from_layer(&studio_core::Layer::new(0));
        layer.mode = mode;
        layer
    }
    ...
    fn speichern_laden_und_keine_auswahl_sind_persistent() {
        ...
        let profile = service.new_profile("laser-a", &layer(studio_core::LayerMode::Cut));
    ...
    fn materialwerte_werden_prozessbezogen_auf_layerentwurf_angewandt() {
        ...
        let mut service = MaterialService::load_from(&dir).unwrap();
        let mut profile = service.new_profile("laser-a", &layer(studio_core::LayerMode::Cut));
        ...
        let mut layers = vec![
            layer(studio_core::LayerMode::Cut),
            layer(studio_core::LayerMode::Fill),
        ];
