...
pub struct PreparedAsset {
    pub meta: studio_core::AssetMeta,
...
impl AssetService {
    ...
    fn is_derived(asset: &studio_core::AssetMeta) -> bool {
    ...
    pub fn import_font(path: &Path) -> Result<studio_core::AssetMeta, AppError> {
        let bytes = std::fs::read(path).map_err(|error| {
            AppError::wrap(
                "font_read",
        ...
        studio_core::text::text_to_contours(&bytes, "Ag", 20.0).map_err(|error| {
            AppError::wrap(
                "font_invalid",
    ...
    pub fn trace_preview(
        ...
        let gray = studio_core::apply_params(&pixels, &lut, false);
    ...
    pub fn crop_preview(
        ...
        let mut processed =
            studio_core::apply_params(cropped.as_raw(), params, params.invert_editor);
        if studio_core::dither::is_dither(params.mode) {
            processed = studio_core::dither::dither(
                &processed,
                width as usize,
                height as usize,
                params.mode,
    ...
    pub fn crop_image(id: &str, crop: [f32; 4]) -> Result<studio_core::AssetMeta, AppError> {
        let cropped = Self::cropped_luma(id, crop)?;
    ...
    ) -> Result<(studio_core::AssetMeta, [f32; 4]), AppError> {
        match geometry {
    ...
    fn store_crop(cropped: image::GrayImage) -> Result<studio_core::AssetMeta, AppError> {
        let mut png = Vec::new();
    ...
    ) -> Result<studio_core::AssetMeta, AppError> {
        let mut png = Vec::new();
    ...
    fn prepare_meta(meta: studio_core::AssetMeta) -> Result<PreparedAsset, AppError> {
        ...
        let contours = match meta.kind {
            studio_core::AssetKind::SvgSource | studio_core::AssetKind::DxfSource => {
                ...
                Some(
                    studio_core::import::import_vector_compounds(&bytes, &meta.ext).map_err(
                        |error| {
                            AppError::wrap(
                                "vector_import",
...
mod tests {
    ...
    fn elliptischer_crop_maskiert_ecken_und_behaelt_mitte() {
        ...
        image::DynamicImage::ImageLuma8(source)
            .write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png)
            .expect("PNG");
        let meta = studio_core::import_image(&studio_core::assets_dir(), &png, "Quelle.png")
            .expect("Quellasset");
        ...
        let (pixels, width, height) =
            studio_core::load_asset_luma(&studio_core::assets_dir(), &cropped.id)
                .expect("Crop laden");
        let (rgba, _, _) = studio_core::load_asset_rgba(&studio_core::assets_dir(), &cropped.id)
    ...
    fn crop_assets_erscheinen_nur_mit_gespeicherter_projektreferenz() {
        ...
        image::DynamicImage::ImageLuma8(source)
            ...
            .unwrap();
        let original =
            studio_core::import_image(&studio_core::assets_dir(), &png, "Original.png").unwrap();
        ...
        assert!(studio_core::asset_meta(&studio_core::assets_dir(), &crop.id).is_ok());
    }

    #[test]
    fn unbekanntes_format_wird_stabil_abgewiesen() {
        let _guard = crate::test_env::with_temp_dir("asset_format");
        let path = studio_core::assets_dir().join("quelle.txt");
    ...
    fn unbrauchbarer_font_wird_nicht_katalogisiert() {
        ...
        let path = studio_core::assets_dir().join("kaputt.ttf");
        std::fs::create_dir_all(path.parent().expect("Datenverzeichnis"))
            .expect("Datenverzeichnis anlegen");
    ...
    fn bildvorschau_nutzt_die_core_parameterpipeline() {
        ...
        image::DynamicImage::ImageLuma8(source)
            .write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png)
            .expect("Test-PNG");
        let meta = studio_core::import_image(&studio_core::assets_dir(), &png, "preview.png")
            .expect("Bild importieren");
        let params = studio_core::ImageParams {
            mode: studio_core::ImageMode::Threshold,
            threshold: 128,
            ..Default::default()
        };
